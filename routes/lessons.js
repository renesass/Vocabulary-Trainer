...
var save = function (req, res, next) {
	...
	Lesson.findOneById(id, function (error, lesson) {
		...
		lesson.save(function (error) {
			...
			let exampleSentenceArray = req.body["exampleSentence[]"];
			let additionalInfoArray = req.body["additionalInfo[]"];
			...
			if (typeof vocabularyIdArray === 'string') {
				...
				exampleSentenceArray = [exampleSentenceArray];
				additionalInfoArray = [additionalInfoArray];
			}

			// delete vocabularies, all of which were removed
			...
			for (i = 0; i < vocabularyIdArray.length; i++) {
				...
				let exampleSentence = exampleSentenceArray[i];
				let additionalInfo = additionalInfoArray[i];

				if (foreign.length != 0 && native.length != 0) {
					Vocabulary.findOneById(vocabularyId, function (error, vocabulary) {
						if (vocabulary == null) vocabulary = new Vocabulary(null, lesson.id, foreign, pronunciation, native, exampleSentence, additionalInfo, 0, 0, 0, 0);
						else {
							vocabulary.foreign = foreign;
							vocabulary.pronunciation = pronunciation;
							vocabulary.native = native;
							vocabulary.exampleSentence = exampleSentence;
							vocabulary.additionalInfo = additionalInfo;
						}
